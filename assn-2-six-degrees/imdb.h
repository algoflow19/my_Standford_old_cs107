...
struct finder{
  deque<string> actorForProcess;
  set<film> processedFilm;
  set<string> processedActor;
  map<string,path> actorOnPath;
  string startPoint;
...
public:
  ...
  string generateShorestPath(const string& s1,const string&s2);
  void printPath(const string& s1);
  void clearPath();
...
private:
  ...
  string sideExecuteMove(finder& mainSide, finder &otherSide);

  finder startSide,endSide;
 ...
 // static map<string,vector<film> > actorCredits;
 // static map<film,vector<string> > moiveCast;

  bool loadFilmInfo(int offset, vector<string> &actorList, int movieNameLenght) const;
  bool loadActorInfo(int offset, vector<film> &filmList, int actorNameLength) const;

  char* getActorName(int offset) const;
  void getMovieInfo(int offset,film & container) const;
